...
export default function registerSocketHandlers(io) {
    ...
    io.on('connection', async (socket) => {
        ...
        // 玩家加入
        socket.on('player:join', async (playerInfo) => {
            try {
        ...
        // 创建房间
        socket.on('room:create', async (roomInfo) => {
            try {
                // 获取或创建玩家
                let player = await playerManager.getPlayer(playerId);
                if (!player) {
        ...
        // 加入房间
        socket.on('room:join', async (joinData) => {
            try {
        ...
        socket.on('room:leave', async (data) => {
            try {
                ...
                console.log(`玩家 ${playerId} 离开房间 ${roomId}, 是否为房主: ${isHost}, 当前房间人数: ${room.players.length}`);
                // 如果是房主离开
                if (isHost) {
